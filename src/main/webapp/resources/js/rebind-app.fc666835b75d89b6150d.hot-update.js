...
var addLoggingToDispatch=function addLoggingToDispatch(store){var rawDispatch=store.dispatch;return function(action){console.group(action.type);console.log('prev_state:',store.getState());console.log('action_type:',action.type);var result=rawDispatch(action);console.log('next_state:',store.getState());console.groupEnd(action.type);return result;};};var loadState=function loadState(){return{authInfo:{logged:false}};};/* eslint-disable no-underscore-dangle */var configureStore=function configureStore(){var INITIAL_STATE=loadState();var store=Object(redux__WEBPACK_IMPORTED_MODULE_0__["createStore"])(_reducers_rootReducer__WEBPACK_IMPORTED_MODULE_1__["default"],INITIAL_STATE,window.__REDUX_DEVTOOLS_EXTENSION__&&window.__REDUX_DEVTOOLS_EXTENSION__());// set subscribtions to store changes.
return store;};/* eslint-enable no-underscore-dangle */var configuredStore=configureStore();

/***/ })

})
//# sourceMappingURL=rebind-app.fc666835b75d89b6150d.hot-update.js.map